...
def get_candidates(run: CampaignRun, campaign_config: CampaignConfig, queue_name: str) -> int:
   ...
   logger.debug("Resolved pending credits test data path: %s (exists=%s)", str(data_file), data_file.exists())
...
def _build_credit_list(
   ...
   for sequence, credit_detail in enumerate(
       ...
       credit_type = (
           credit_detail.get("CREDIT_TYPE")
           or "Trade In"
       )

       credit_name = (
           credit_detail.get("CREDIT_NAME")
           or credit_type
       )
       ...
       credit_list.append(
           {
               ...
               "creditAmount": float(
                   credit_detail.get("CREDIT_AMOUNT", 0) or 0
               ),
               "lineLast4": (
                   phone_number[-4:]
                   if phone_number
...
def _build_email_payload(
   ...
   ban = (
       work.source_context.get("BAN", "")
       if work.source_context
       else ""
   )

   customer_platform = (
       work.source_context.get("PLATFORM_HANDLER")
       if work.source_context
       else None
   ) or "MyATT"
