...
(TDD Sections 8.1, 8.2, 9). Serves all four campaigns from one queue by
reading campaign_id off each message.
"""
...
def process_bill_variance(message: func.ServiceBusMessage) -> None:
    ...
    # Any exception here abandons the message, so Service Bus retries and
    # eventually dead-letters it after max delivery count is exceeded.
    handler(work)
